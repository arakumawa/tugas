...
include "koneksi.php";
$id_produk = $_GET['id'];
$a = mysql_query("SELECT id_produk, nama_produk, id_kategori, harga, keterangan FROM produk WHERE id_produk = '$id_produk' ");
...
<div class="tambah-data" style="width:325px; height:200px; border:1px solid #123ABC; background-color: #4D5B67;">
<form method="POST" action="index.php?page=produk_proses">
	...
	<table width="100%" style="margin-left:10px; margin-top:5px;">
			...
			<td>
				<select name="cbkategori">
					...
					while($hasil_kat = mysql_fetch_array($kat)){
						...
						if($hasil['id_kategori']==$hasil_kat['id_kategori']) echo " selected";
		...
		<tr>
			...
			<td>Rp.<input name="txtharga" value="<?php echo $hasil['harga'];?>" onkeyup="javascript:if(isNaN(this.value)){this.value='0'; whis.select();}"/></td>
		</tr>
		<tr>
...
<table class="table" border="1">
	...
	$a = mysql_query("SELECT A.id_produk, A.nama_produk, B.nama_kategori, A.harga , A.keterangan 
	FROM produk AS A
